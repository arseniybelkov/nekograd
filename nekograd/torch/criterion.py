...
from nekograd.itertools import mapf


class CriterionDict(nn.Module):
    """
    Parameters
    ----------
    criterions: Callable
    ...
    scale: bool
        ...
        Default: False
    """
    def __init__(self, *criterions: Callable,
        ...
        assert len(alphas) == len(criterions), f'Got {len(alphas)} alphas\
                                            and {len(criterions)} criterions,\
                                            but the numbers must be the same'
        if loss_keys is not None:
            loss_keys = loss_keys if isinstance(loss_keys, (list, tuple)) else [loss_keys]
        else:
            loss_keys = [c.__name__ if isfunction(c) else c.__class__.__name__ for c in criterions]
            
        assert len(set(loss_keys)) == len(loss_keys), f'There are criterions with simmilar names: {loss_keys}'
        assert len(loss_keys) == len(criterions), f'Got {len(loss_keys)} loss_keys\
                                            and {len(criterions)} criterions,\
                                            but the numbers must be the same'
    ...
    def forward(self, *args, **kwargs) -> Dict[str, torch.Tensor]:
        loss = {'total': 0}
        for _loss, alpha, loss_key in zip(mapf(self.criterions, *args, **kwargs), self.alphas, self.loss_keys):
            if isinstance(_loss, (int, float)):
                _loss = torch.tensor(_loss, dtype=torch.float32).cuda()
            loss['total'] += _loss * alpha
